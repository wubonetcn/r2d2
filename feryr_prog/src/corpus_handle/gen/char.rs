...
pub fn gen_char(_ctx: &mut Context, rng: &mut RngType, ty: &Type, dir: Dir) -> Value {
    let ty = ty.checked_as_char();
    let bit_sz = ty.bit_size();

    if let Some(range) = ty.range().cloned() {
        if rng.gen_ratio(99, 100) {
            let val = rand_char_range(rng, range, ty.align(), bit_sz);
            return CharValue::new(ty.id(), dir, val).into();
        }
    }
    let val = rand_char_in_bit_sz(rng, bit_sz);
    CharValue::new(ty.id(), dir, val).into()
}
...
fn rand_char_in_bit_sz(rng: &mut RngType, bit_sz: u64) -> u64 {
    const GENERATORS: [fn(&mut RngType) -> u64; 3] = [favor_range, special_char, rand_char];
...
fn rand_char(rng: &mut RngType) -> u64 {
    rng.gen()
}
...
#[inline]
fn special_char(rng: &mut RngType) -> u64 {
